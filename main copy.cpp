...
static const char* PATH_WPS =
    "C:\\Program Files\\Kingsoft\\WPS Office\\12.1.0.24655\\office6\\wpp.exe";
...
static std::string QuickGetTag(const char* src, size_t len, const char* tag) {
    ...
    const char* p = static_cast<const char*>(
        memmem_impl(src, len, sTag, static_cast<size_t>(sLen)));
    // memmem 不是 MSVC 标准库函数，用 strstr 替代（src 是解压出的 XML，以 \0 结尾）
    p = strstr(src, sTag);
    if (!p) return {};
    p += sLen;
